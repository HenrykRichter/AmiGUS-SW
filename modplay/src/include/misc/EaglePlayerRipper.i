...
RPT_TagBase	EQU	TAG_USER+"ER"
	...
	EITEM	RPT_Ripp1		;Ptr to Ripp Pass1
					...
					*	 d1=Size (unknown=-1 -> d1=(EndAdr-CurrentAdr))
					*	 a0=Startadr (data)
			*#####################################################
			*Attention!!! Don't change the following registers:
			*	  d3-d7/a3-a6 !!!!!!!!!!!!!!!!!!!!!!!
			*
			*#####################################################
	EITEM	RPT_Ripp2		;Ptr to Ripp Pass2 (Input/Output see RPT_Ripp1)
	EITEM	RPT_Ripp3		;Ptr to Ripp Pass3 (Input/Output see RPT_Ripp1)
	EITEM	RPT_ExtRipp		;for Samplesearch (not implemented yet)
	...
	EITEM	RPT_ExtSave		* Saves extended data
					...
					*  a0=start of memory             
					*  d0=size of memory                   
					*  a1=modulestart (returned by RPT_Rippx)                    
					*  d1=modulessize (returned by RPT_Rippx (corrected if it was -1))
					*  a2=Ripperstruct              
					* 0utput:d0=Error or NULL     
					*--------------------------------
...
*---------- Ripperstruct (filled by Engine after calling EUT_Ripp) -----------*
