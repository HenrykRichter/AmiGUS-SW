...
**	(will be changed if prefs are loaded :)-=-=	)			**
 ...
 STRUCTURE Engine_START,0
	...
	APTR	EUS_AMTiming		;Amplifier supplied playback timing (i.e. no separate timer/interrupt needed)
...
*------------------------ Amplifiers timing provisions -----------------------*
 STRUCTURE Amplifier_Timing,0
        ULONG   APT_Version     ;        extension, keep 0 for now
        FPTR    APT_Init        ;        optional, can be NULL
        FPTR    APT_SetTime     ;        required, put a valid pointer here
        FPTR    APT_End         ;        optional, can be NULL
...
EPNrT_Amplifier		EQU	3
EPNrT_MaxId		EQU	3
EPNrT_OnOffset          EQU     1               ;avoid 0 for active timer in TimerOn
...
EPAMF_DirectInts	EQU	8 ;Replayer Audio Interrupt should be called directly instead of signaling for after mixing (per channel in AS_SampleFlags)
...
EPAMB_DirectInts	EQU	1<<EPAMF_DirectInts
