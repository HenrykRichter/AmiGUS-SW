...
*****************************************************************
		...
		XDEF	_PT_PlayTick
		XDEF	_PT_SetDryRun
		XDEF	_PT_DryChannelState
		XDEF	_PT_DryDMAState
		XDEF	_PT_DryCounterSpeedPposSpos
		XDEF	_PT_DryPatternEntry
...
_PT_DryChannelState:
	move.l	#pt_DRY_FakeChan0,d0
	rts

_PT_DryDMAState:
	moveq	#0,d0
	move.w	pt_DRY_DMAState,d0
	rts

_PT_DryCounterSpeedPposSpos:
	MOVE.L	pt_Counter(PC),D0
	lsl.l	#8,d0
	move.b	pt_CurrSpeed+3,d0	;move.b	pt_Speed,d0
	swap 	d0

	move.l	pt_PatternPosition,d1
	lsr.l	#4,d1
	move.b	d1,d0			;0...63
	lsl.w	#8,d0

	move.b	pt_SongPosition+3,d0
	rts

_PT_DryPatternEntry:
	move.l pt_PatternPtr_dry,d0
	rts
...
pt_lop3 MOVE.L	A2,(A1)+
	...
	move.w	#330,pt_timeout

	lea	pt_DRY_FakeChan0,a0
	move.b	pt_DryRun(pc),d0
	bne.s	.dry1
	LEA	$DFF0A0,A0
	OR.B	#2,$BFE001
.dry1:
...
;	MOVE.B	#$FF,pt_LowMask		;buggs: this would fix the first portamento slide per Mod 
	CLR.B	pt_SongPos
	CLR.L	pt_Counter
	CLR.W	pt_PattPos
...
pt_GetNewNote
	...
	lea	0(a0,D1.L),a5
	MOVE.L	a5,pt_PatternPtr_dry		;Dry Run debug: Pointer to actual pattern
...
pt_dmaupdate_dry:
;d0: dma bits $8000 = set mode, $0000 = clr mode
	bclr	#15,d0
	bne.s	.set
	not.w	d0	;clear mode
	and.w	pt_DRY_DMAState(pc),d0
	move.w	d0,pt_DRY_DMAState
	rts
.set:
	or.w	pt_DRY_DMAState(pc),d0
	move.w	d0,pt_DRY_DMAState
	rts
...
pt_DryRun:	dc.b	0
		cnop	0,4
pt_PatternPtr_dry:	dc.l	0
;/* faked audio channels in DryRun mode */
pt_DRY_FakeChan0:	ds.b	$10
pt_DRY_FakeChan1:	ds.b	$10
pt_DRY_FakeChan2:	ds.b	$10
pt_DRY_FakeChan3:	ds.b	$10
pt_DRY_DMAState:	ds.w	1
